...
namespace Foam
{
    defineTemplateTypeNameAndDebug(Cloud<uniGasParcel>, 0);
};

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::uniGasCloud::buildConstProps()
{
    ...
    forAll(typeIdList_, i)
    {
        ...
        constProps_[i] = uniGasParcel::constantProperties(molDict);
    }
}


void Foam::uniGasCloud::removeElectrons()
{
...
void Foam::uniGasCloud::addElectrons()
{
...
void Foam::uniGasCloud::weighting()
{
...
void Foam::uniGasCloud::adaptation()
{
    if (adaptive())
    {
...
void Foam::uniGasCloud::decomposition()
{
    ...
    {
        hybridDecompositionModel_->decompose();
    }
}

// * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * * //

void Foam::uniGasCloud::addNewParcel
(
...
{
    uniGasParcel* pPtr = new uniGasParcel
    (
        mesh_,
        position,
        U,
...
Foam::scalar Foam::uniGasCloud::energyRatio
(
    const scalar ChiA,
    const scalar ChiB
)
...
Foam::scalar Foam::uniGasCloud::PSIm
(
    const scalar DOFm,
    const scalar DOFtot
)
...
Foam::labelList Foam::uniGasCloud::getTypeIDs(const dictionary& dict) const
{
...
void Foam::uniGasCloud::evolve()
{

    uniGasParcel::trackingData td(*this);
    ...
    Cloud<uniGasParcel>::move(*this, td, mesh_.time().deltaTValue());
...
Foam::label Foam::uniGasCloud::nTerminalOutputs()
{
    return nTerminalOutputs_;
}


void Foam::uniGasCloud::info() const
{
    label nUniGasParticles = this->size();
    reduce(nUniGasParticles, sumOp<label>());

    scalar nMol = nUniGasParticles*nParticle_;
    ...
    Info<< "    Number of particles             = " << nUniGasParticles << endl;
...
void Foam::uniGasCloud::loadBalanceCheck()
{
    dynamicLoadBalancing_.calculate();
}

Foam::vector Foam::uniGasCloud::equipartitionLinearVelocity
(
...
Foam::vector Foam::uniGasCloud::chapmanEnskogLinearVelocity
(   
...
Foam::scalar Foam::uniGasCloud::equipartitionRotationalEnergy
(
    const scalar temperature,
...
Foam::label Foam::uniGasCloud::equipartitionElectronicLevel
(
    const scalar temperature,
...
Foam::scalar Foam::uniGasCloud::postCollisionRotationalEnergy
(
    const scalar rotationalDof,
    const scalar ChiB
)
...
Foam::label Foam::uniGasCloud::postCollisionVibrationalEnergyLevel
(
...
Foam::label Foam::uniGasCloud::postCollisionElectronicEnergyLevel
(
    const scalar Ec,
    const label jMax,
    const scalar omega,
...
void Foam::uniGasCloud::dumpParticlePositions() const
{
    OFstream pObj
    (
...
void Foam::uniGasCloud::cellWeighting()
{
    const auto& cellOccupancy = this->cellOccupancy();
...
void Foam::uniGasCloud::axisymmetricWeighting()
{
    const auto& cellOccupancy = this->cellOccupancy();
...
void Foam::uniGasCloud::axisymmetricCellWeighting()
{
    const auto& cellOccupancy = this->cellOccupancy();
...
void Foam::uniGasCloud::cleanLagrangian()
{
