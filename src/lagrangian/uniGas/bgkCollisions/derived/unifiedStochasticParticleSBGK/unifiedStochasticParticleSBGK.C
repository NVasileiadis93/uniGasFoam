...
#include "unifiedStochasticParticleSBGK.H"
...
namespace Foam
{
defineTypeNameAndDebug(unifiedStochasticParticleSBGK, 0);

addToRunTimeSelectionTable(bgkCollisionModel, unifiedStochasticParticleSBGK, dictionary);
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::unifiedStochasticParticleSBGK::unifiedStochasticParticleSBGK
(
...
void Foam::unifiedStochasticParticleSBGK::calculateProperties()
{
    ...
    forAll(mesh_.cells(), cell)
    {
        ...
        shearStressTensor_[cell] = theta_*shearStressTensor_[cell]/(1.0+0.5*collFreq_[cell]*deltaT) + (1.0-theta_)*shearStressTensorPrevious_[cell];
...
void Foam::unifiedStochasticParticleSBGK::resetProperties()
{
...
void Foam::unifiedStochasticParticleSBGK::collide()
{
    ...
    autoPtr <Foam::interpolation<scalar>> collFreqInterp;
                ...
                if (macroInterpolation_)
                {
                    parcel.U() = samplePostCollisionVelocity
                            (
                                ...
                                collFreqInterp().interpolate(position, cell),
...
void Foam::unifiedStochasticParticleSBGK::conserveMomentumAndEnergy
(
    const label& cell
)
{
...
Foam::vector Foam::unifiedStochasticParticleSBGK::samplePostCollisionVelocity
(   
    const label& cell,
    const scalar& m,
    const scalar& Pr,
...
Foam::unifiedStochasticParticleSBGK::propertiesDict() const
{
