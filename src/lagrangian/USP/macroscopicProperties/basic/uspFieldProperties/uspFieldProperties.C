...
Foam::uspFieldProperties::uspFieldProperties
(
    const Time& t,
    const polyMesh& mesh
)
...
:
    ...
    fieldList_(uspFieldPropertiesDict_.lookup("uspFields")),
    ...
    fields_(fieldList_.size())
{
    if (fields_.size() > 0)
    {
        ...
        forAll(fields_, f)
        {
            ...
            fields_[f] = uspField::New(time_, mesh, cloud, fieldIDict);
...
void Foam::uspFieldProperties::createFields()
{
...
void Foam::uspFieldProperties::writeFields()
{
