...
addToRunTimeSelectionTable(uspReaction, atomIonIonisation, dictionary);
}
...
void Foam::atomIonIonisation::reaction(uspParcel& p, uspParcel& q)
{
...
{
    bool write = uspReaction::outputResults(counterIndex);
    ...
    if (write)
    {
        ...
        {
            for (uspParcel* p : cellOccupancy[c])
            {
