...
Foam::uspAllConfigurations::uspAllConfigurations
(
    const IOdictionary& uspInitialiseDict,
    uspCloud& cloud
)
:
    uspInitialiseDict_(uspInitialiseDict),
    configurationList_(uspInitialiseDict_.lookup("configurations")),
    ids_(configurationList_.size()),
    configurations_(configurationList_.size())
{
    Info << nl << "Creating usp configurations: " << nl << endl;
    ...
    if (configurations_.size() > 0)
    {
        forAll(configurations_, c)
        {
            ...
            configurations_[c] =
                uspConfiguration::New(cloud, configurationIDict);
...
void Foam::uspAllConfigurations::setInitialConfig()
{
    forAll(configurations_, c)
    {
