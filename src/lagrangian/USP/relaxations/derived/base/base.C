...
namespace Foam
{
defineTypeNameAndDebug(base, 0);

addToRunTimeSelectionTable(relaxationModel, base, dictionary);
}
...
void Foam::base::calculateProperties()
{
    ...
    forAll(mesh_.cells(), cell)
    {
        if (rhoNMean_[cell] > VSMALL)
        {
            ...
            // Shear stress tensor
            scalar pressureTensorTrace = (pressureTensor_[cell].xx() + pressureTensor_[cell].yy() + pressureTensor_[cell].zz())/3.0;

            shearStressTensor_[cell] = -pressureTensor_[cell];
            shearStressTensor_[cell].xx() += pressureTensorTrace;
            shearStressTensor_[cell].yy() += pressureTensorTrace;
            shearStressTensor_[cell].zz() += pressureTensorTrace;
    ...
    if (timeAverage_)
    {
        forAll(mesh_.cells(), cell)
        {
            heatFluxVector_[cell] = 
                theta_*heatFluxVector_[cell]/(1.0+0.5*Prandtl_[cell]*relaxFreq_[cell]*cloud_.mesh().time().deltaTValue()) + (1.0-theta_)*heatFluxVectorPrevious_[cell];
            pressureTensor_[cell] = 
                theta_*pressureTensor_[cell]/(1.0+0.5*relaxFreq_[cell]*cloud_.mesh().time().deltaTValue()) + (1.0-theta_)*pressureTensorPrevious_[cell];
            shearStressTensor_[cell] = 
                theta_*shearStressTensor_[cell]/(1.0+0.5*relaxFreq_[cell]*cloud_.mesh().time().deltaTValue()) + (1.0-theta_)*shearStressTensorPrevious_[cell];
            heatFluxVectorPrevious_[cell] = heatFluxVector_[cell];
            pressureTensorPrevious_[cell] = pressureTensor_[cell];
    ...
    else
    {
        forAll(mesh_.cells(), cell)
        {
            heatFluxVector_[cell] = heatFluxVector_[cell]/(1.0+0.5*Prandtl_[cell]*relaxFreq_[cell]*cloud_.mesh().time().deltaTValue());
            pressureTensor_[cell] = pressureTensor_[cell]/(1.0+0.5*relaxFreq_[cell]*cloud_.mesh().time().deltaTValue());
            shearStressTensor_[cell] = shearStressTensor_[cell]/(1.0+0.5*relaxFreq_[cell]*cloud_.mesh().time().deltaTValue());
        }
    }
...
void Foam::base::resetProperties()
{
...
void Foam::base::conserveMomentumAndEnergy
(
    const label& cell
)
{
...
Foam::vector Foam::base::samplePostRelaxationVelocity
(   
    const label& cell,
    const scalar& m,
    const scalar& Pr,
...
)
{
    ...
    while(true)
    {
        ...
        //v = cloud_.rndGen().GaussNormal<vector>()*sqrt(physicoChemical::k.value()/m*T);
        //v2 = sqr(v.x())+sqr(v.y())+sqr(v.z());
        //Info << p << " " << physicoChemical::k.value()/m << " " << T << " " << v2 << " " << endl;
        //prob = 1.0+(1.0-Pr)/(5.0*p*physicoChemical::k.value()/m*T)*(q.x()*v.x()+q.y()*v.y()+q.z()*v.z())*(v2/(physicoChemical::k.value()/m*T)-5.0);
...
Foam::base::propertiesDict() const
{
