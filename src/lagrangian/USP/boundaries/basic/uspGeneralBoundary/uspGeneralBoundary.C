...
namespace Foam
{
defineTypeNameAndDebug(uspGeneralBoundary, 0);
defineRunTimeSelectionTable(uspGeneralBoundary, dictionary);
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::uspGeneralBoundary::uspGeneralBoundary
(
    const polyMesh& mesh,
    uspCloud& cloud,
...
:
    uspBoundaryBase(mesh, cloud, dict, "general"),
    faces_(),
...
Foam::autoPtr<Foam::uspGeneralBoundary> Foam::uspGeneralBoundary::New
(
    const polyMesh& mesh,
    uspCloud& cloud,
    const dictionary& dict
)
{
    ...
    Info<< "Selecting uspGeneralBoundaryModel " << modelType << nl;
    ...
    return autoPtr<uspGeneralBoundary>(cstrIter()(mesh, cloud, dict));
}
    ...
    forAll(faces_, f)
    {
        ...
        forAll(typeIds_, iD)
        {
            ...
            for (label i = 0; i < nParcelsToInsert; ++i)
            {
                ...
                // Equivalent to the QA value in Bird's USP3.FOR
...
const Foam::labelList& Foam::uspGeneralBoundary::controlPatch() const
{
    return faces_;
}
